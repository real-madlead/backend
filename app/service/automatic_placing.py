...
class Room:
    ...
    def plot_room(self):
        ...
        last_line = {
            "x": [last_ps[0][0], last_ps[1][0]],
            "y": [last_ps[0][1], last_ps[1][1]],
        }
        ...
        for i in range(len(points) - 1):
            ...
            lines.append(line)
        for line in lines:
            pseudogeometric_line_for_calculation = create_line(
    ...
    def plot_furniture(self, furniture_places_list: list):
        ...
        for furniture_place in furniture_places_list:
            calculate_furniture = create_rectangle(
                [furniture_place.x, furniture_place.y],
                furniture_place.width,
                furniture_place.length,
                furniture_place.rotation,
                "blue",
            )

            if (
                (multi_check_overlap(calculate_furniture, self.line_objects))
                or (furniture_place.x <= min_x)
                or (furniture_place.x >= max_x)
                or (furniture_place.y <= min_y)
                or (furniture_place.y >= max_y)
            ):
                error_flag.append(1)
            ...
            for furniture_object in sorted_furniture_objects_list:
                ...
                while True:
                    if furniture_object.restriction == "alongwall":
                        x, y, rotation = set_alongwall(
                            min_x,
                            max_x,
                            min_y,
                            max_y,
                            furniture_object.rand_rotation,
                            delta=0.01,
                        )
                    elif furniture_object.restriction == "alongwall direction center":
                        x, y, rotation = set_alongwall_dir_ctr(
                        ...
                        furnitureplace_objects_targeted_for_set = [
                            i
                            for i in furnitureplace_objects_list
                            if i.name == furniture_object.restriction.split("_")[1]
                        ]
                        if len(furnitureplace_objects_targeted_for_set) != 0:
                            x, y, rotation = set_combo(
                                furniture_object.length,
                            ...
                            x, y, rotation = (
                                ...
                                random.choice(furniture_object.rand_rotation),
                            )
                    elif furniture_object.restriction.split("_")[0] == "facing":
                        furnitureplace_objects_targeted_for_facing = [
                            i
                            for i in furnitureplace_objects_list
                            if i.name == furniture_object.restriction.split("_")[1]
                        ]
                        if len(furnitureplace_objects_targeted_for_facing) != 0:
                            x, y, rotation = set_facing(
                                furniture_object.length,
                                furniture_object.width,
                                min_x,
                                max_x,
...
def set_alongwall(min_x, max_x, min_y, max_y, rand_rotation, delta=0.01):
    ...
    if rand == 0:
        x, y = random.choice([min_x + delta, max_x - delta]), random.uniform(
            min_y, max_y
        )
    elif rand == 1:
        x, y = random.uniform(min_x, max_x), random.choice(
            [min_y + delta, max_y - delta]
        )
    return x, y, rotation


def set_combo(
    v_width, h_width, min_x, max_x, min_y, max_y, set_furniture=None, delta=0.01
):
    ...
    x, y, rotation : float, float, int
        配置する家具のx, y座標と角度(度数法)
    """
    set_f_x, set_f_y, set_f_rotation = (
        set_furniture.x + delta * math.sin(math.radians(set_furniture.rotation)),
        set_furniture.y - delta * math.cos(math.radians(set_furniture.rotation)),
        set_furniture.rotation,
    )
    rand = random.choice([0, 1, 2, 3])
    if rand == 0:
        ...
        x = (
            set_f_x
            + set_f_rand_len * math.cos(math.radians(set_f_rotation))
            + v_width * math.cos(math.radians(set_f_rotation))
            + h_width * math.sin(math.radians(set_f_rotation))
        )
        y = (
            set_f_y
            + set_f_rand_len * math.sin(math.radians(set_f_rotation))
            + v_width * math.sin(math.radians(set_f_rotation))
            - h_width * math.cos(math.radians(set_f_rotation))
        )
        ...
        x = (
            set_f_x
            + set_furniture.width * math.cos(math.radians(set_f_rotation))
            - set_f_rand_len * math.sin(math.radians(set_f_rotation))
            + h_width * math.cos(math.radians(set_f_rotation))
            - v_width * math.sin(math.radians(set_f_rotation))
        )
        y = (
            set_f_y
            + set_furniture.width * math.sin(math.radians(set_f_rotation))
            + set_f_rand_len * math.cos(math.radians(set_f_rotation))
            + h_width * math.sin(math.radians(set_f_rotation))
            + v_width * math.cos(math.radians(set_f_rotation))
        )
        ...
        x = (
            ...
            + set_f_rand_len * math.cos(math.radians(set_f_rotation))
            - h_width * math.sin(math.radians(set_f_rotation))
            - set_furniture.length * math.sin(math.radians(set_f_rotation))
        )
        y = (
            set_f_y
            + set_f_rand_len * math.sin(math.radians(set_f_rotation))
            + h_width * math.cos(math.radians(set_f_rotation))
            + set_furniture.length * math.cos(math.radians(set_f_rotation))
        )
        rotation = set_f_rotation - 90
    elif rand == 3:
        set_f_rand_len = random.uniform(0, set_furniture.length - v_width)
        x = -1 * set_f_rand_len * math.sin(
            math.radians(set_f_rotation)
        ) - h_width * math.cos(math.radians(set_f_rotation))
        y = set_f_rand_len * math.cos(
            math.radians(set_f_rotation)
        ) - h_width * math.sin(math.radians(set_f_rotation))
...
def set_facing(
    v_width, h_width, min_x, max_x, min_y, max_y, face_furniture, delta=0.01
):
    ...
    face_x, face_y, face_h, face_v = (
        face_furniture.x,
        face_furniture.y,
        face_furniture.width,
        face_furniture.length,
    )
    if face_rotation == 0:
        x, y, rotation = (
            random.uniform(face_x + h_width + face_h, max_x),
            face_y + face_v / 2 + v_width / 2,
            180,
        )
    elif face_rotation == 90:
        x, y, rotation = (
            face_x - face_v / 2 - v_width / 2,
            random.uniform(face_y + h_width + face_h, max_y),
            270,
        )
    elif face_rotation == 180:
        x, y, rotation = (
            random.uniform(min_x, face_x - face_h - h_width),
            face_y - face_v / 2 - v_width / 2,
            0,
        )
        ...
        x, y, rotation = (
            face_x + face_v / 2 + v_width / 2,
            random.uniform(min_y, face_y - h_width - face_h),
            90,
        )
    return x, y, rotation
...
def generate_room(
    ...
    for I in range(generate_num):
        ...
        room.plot_room()
        furnitureplace_list = room.place_furnitures_with_restriction(
            furniture_objects_list=furniture_list
        )
        rooms_furniture_placement_list.append(furnitureplace_list)
    rooms_furniture_placement_df = convert_furniture_list_to_dataframe(
        rooms_furniture_placement_list, floor_object=floor_object
    )
    return rooms_furniture_placement_df


def convert_furniture_list_to_dataframe(
    rooms_furniture_placement_list: list[list[FurniturePlace]], floor_object: Floor
):
    ...
    for index, furniture_placement_list in enumerate(rooms_furniture_placement_list):
        ...
        furniture_placement_dict = {
            ...
            "room_v": floor_object.length,
            "room_h": floor_object.width,
            "target": "uninspected",
        }
        ...
        for furniture in furniture_data:
            specific_furniture_placements_list = [
                ...
                if furniture_placement.name == furniture.name
            ]
        ...
        for furniture in furniture_data:
            for i in range(3):
                if f"{furniture.name}_{i+1}" in only_name_list:
                    ...
                    furniture_placement_dict[f"{furniture.name}_{i+1}_v_width"] = next(
                        (
                            d["length"]
                            for d in dummy_dict_list
                            if d["name"] == f"{furniture.name}_{i+1}"
                        ),
                        None,
                    )
                    furniture_placement_dict[f"{furniture.name}_{i+1}_h_width"] = next(
                        (
                            d["width"]
                            for d in dummy_dict_list
                            if d["name"] == f"{furniture.name}_{i+1}"
                    ...
                    furniture_placement_dict[f"{furniture.name}_{i+1}_x"] = next(
                        (
                            d["x"]
                            for d in dummy_dict_list
                            if d["name"] == f"{furniture.name}_{i+1}"
                    ...
                    furniture_placement_dict[f"{furniture.name}_{i+1}_y"] = next(
                        (
                            d["y"]
                            for d in dummy_dict_list
                            if d["name"] == f"{furniture.name}_{i+1}"
                    ...
                    furniture_placement_dict[f"{furniture.name}_{i+1}_rotation"] = next(
                        (
                            d["rotation"]
                            for d in dummy_dict_list
                            if d["name"] == f"{furniture.name}_{i+1}"
                    ...
                    for fur in furniture_data:
                        for i_2 in range(3):
                            ...
                            ] = get_furniture_distance(
                                dict_list=dummy_dict_list,
        ...
        rooms_furniture_placement_df = pd.concat(
            [rooms_furniture_placement_df, furniture_placement_df], ignore_index=True
        )
...
def get_position(name: str, name_counter: dict, series):
    ...
    x, y, rotation = (
        series[f"""{cur_name}_x"""],
        series[f"""{cur_name}_y"""],
        series[f"""{cur_name}_rotation"""],
    )
    return x, y, rotation
...
def recommend_furniture_using_AI(
    ...
    edges = [
        ...
        [
            current_floor_plan_output_schema.floor.width,
            current_floor_plan_output_schema.floor.length,
        ],
    ...
    while True:
        ...
        for candidate_furniture in candidate_furnitures_for_additional_placement:
            additinal_furnitre_placement_list = (
                copy_current_room.place_furnitures_with_restriction(
                    furniture_objects_list=[candidate_furniture]
                )
            )
            candidate_furnitureplace_list.append(additinal_furnitre_placement_list)

            copy_current_floor_plan_output_schema = copy.deepcopy(
                current_floor_plan_output_schema
            )
            copy_current_floor_plan_output_schema.furnitures += (
                additinal_furnitre_placement_list
            )
            candidate_room_furnitureplace_list.append(
                copy_current_floor_plan_output_schema.furnitures
            )

        test_df = convert_furniture_list_to_dataframe(
            rooms_furniture_placement_list=candidate_room_furnitureplace_list,
            floor_object=current_floor_plan_output_schema.floor,
        )

        # 　AIによる採点
        test_df = test_df.drop(["room_num", "target"], axis=1)
        best_index, best_score = get_high_score_indices(
            model_path="./AI_model/torch_model.pth", test_df=test_df
        )
...
def recommend_many_furniture_using_AI(
    ...
    for candidate_furniture in candidate_furnitures_for_additional_placement:
        while True:
            ...
            test_df = convert_furniture_list_to_dataframe(
                rooms_furniture_placement_list=room_info_list,
                floor_object=current_floor_plan_output_schema.floor,
            )
            # 　AIによる採点
            test_df = test_df.drop(["room_num", "target"], axis=1)
            ...
            if (
                current_floor_plan_output_schema.score_of_room_layout_using_AI
                <= best_score
            ):
                recommend_furnitureplace = candidate_furnitureplace_list[best_index]
                recommend_furnitureplaces_list_each_candidate_furniture.append(
                    recommend_furnitureplace
                )
                recommend_furnitureplaces_score_list.append(best_score)
                break
    # 値とそのインデックスを組にしてソート
    sorted_recommend_furnitureplaces_score_list = sorted(
        [(x, i) for i, x in enumerate(recommend_furnitureplaces_score_list)],
        reverse=True,
    )
    ...
    top_n_furnitureplaces_list = [
        recommend_furnitureplaces_list_each_candidate_furniture[index]
        for index in top_n_index_list
    ]
    ...
    for one_of_top_furnitureplace in top_n_furnitureplaces_list:
        list_of_colorcodes_furnitureplace_object_possesses = furniture_color_data[
            one_of_top_furnitureplace.name
        ]
        optimized_furniture_colorcode = closest_color(
            chatgpt_recommend_color_code,
            list_of_colorcodes_furnitureplace_object_possesses,
        )
